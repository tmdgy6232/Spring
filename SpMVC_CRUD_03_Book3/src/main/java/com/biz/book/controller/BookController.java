...
public class BookController {
	...
	public String book_input(@ModelAttribute BookVO bookVO, Model model) {
		...
		int let = bService.insert(bookVO);
...
//		
		 ...
		 * 그래서 그 method()를 호출하여 같은 일을 수행하려고 했더니
		 * 문제가 발생하더라
		 * 
		 * spring 환경에서는 그럴경우
		 * response에
		 * 웹브라우저에게 해당 url을 다시 request하도록 요청할 수 있다.
		 * 그 키워드가 redirect:....다.
		 */
		//브라우져야
		//주소창에 ...book-view라고 입력하고 엔터를 눌러다오
		if(let>0) {
	 ...
	 * 변수 이름과 같은 형태의 매개변수를 지정하여
	 * 받는다.
	 * 이때 변수는 거의 대부분 String 형이다
	 * String 변수 형식으로 매개변수를 지정한다
	 * 
	 * 단, 실제 사용할 변수의 형 (type)이 문자열이 아닐 경우
	 * 문자열 => 사용할 타입으로 변환이 가능하면
	 * 사용할 type으로 변수의 형을 지정할 수 있다.
	 * 
	 * 이대는 가급적 @RequestParam을 선언하여준다.
	 * 그러면 내부적으로 자동 형변환을 하여 값을 수신할 수 있다.
	 * 하지만 만약 보내는 값이 형변환에 적합하지 않으면
	 * Server는 400오류를 reponse한다.
	 */
	@RequestMapping(value = "book-detail", method=RequestMethod.GET)
	public String book_dedail(
	...
	public String book_delete(Model model, @RequestParam long b_seq) {
	...
	public String update(Model model, @RequestParam long b_seq) {
	...
	public String book_update(@ModelAttribute BookVO bookVO, Model model) {
		...
		int let = bService.update(bookVO);
